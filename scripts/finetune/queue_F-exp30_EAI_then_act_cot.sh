...
CONFIG_FILE="F-exp30-EAI-lerobot_motion_delta005_s5.yaml"
RUN_LABEL="F-exp30-EAI-lerobot_motion_delta005_s5"
TRAIN_PORT="${TRAIN_PORT:-26032}"
...
QUEUE_LOG="${QUEUE_LOG:-logs/aloha_fexp/queue_F-exp30_EAI_then_act_cot.log}"
exec > >(tee -a "$QUEUE_LOG") 2>&1
echo "[$(date '+%Y-%m-%d %H:%M:%S')] queue log=${QUEUE_LOG}"

if [[ ! -f "$ACT_HOLD_PATH" && -f "$ACT_COT_FALLBACK" ]]; then
  echo "[$(date '+%Y-%m-%d %H:%M:%S')] ${ACT_HOLD_PATH} not found; using ${ACT_COT_FALLBACK}" >&2
