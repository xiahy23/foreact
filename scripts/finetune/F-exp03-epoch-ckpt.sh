#!/usr/bin/env bash
# =============================================================
# F-exp03: Based on F-exp01 with per-epoch checkpoint saving
#   - Same hyperparams as F-exp01 (lr=7.1e-6, bs=32x1, cosine_with_min_lr)
#   - save_strategy=epoch (save checkpoint every epoch)
#   - resume_from_checkpoint: finetune_alleval_bridge_offset6/checkpoint-17690
#   - Output: checkpoints/F-exp03-epoch-ckpt
# =============================================================
set -uo pipefail

WORK_DIR="/media/raid/workspace/xiahongyu/foreact"
CONFIG="F-exp03-epoch-ckpt.yaml"
LOG_FILE="${WORK_DIR}/F-exp03-epoch-ckpt.log"
NUM_GPUS="${1:-8}"
MIN_FREE_MB="${2:-70000}"
...
# ---- Launch training ----
echo "[$(date)] Starting F-exp03: config=${CONFIG}, save_strategy=epoch, gpus=${NUM_GPUS}"
