#!/usr/bin/env bash
# =============================================================
# F-exp05: ForeAct default config + latest data/sampling
#   - Default hyperparams (lr=1e-5, constant_with_warmup, freeze vae+mllm_backbone)
#   - Latest data: bridge + all_eval, balanced, offset=6
#   - resume_from_checkpoint: foreact-pretrained
#   - 5 epochs, save_strategy=epoch
#   - Output: checkpoints/F-exp05-default-pretrained-5epoch
# =============================================================
set -uo pipefail

WORK_DIR="/media/raid/workspace/xiahongyu/foreact"
CONFIG="F-exp05-default-pretrained-5epoch.yaml"
LOG_FILE="${WORK_DIR}/F-exp05-default-pretrained-5epoch.log"
NUM_GPUS="${1:-8}"
MIN_FREE_MB="${2:-40000}"
...
# ---- Launch training ----
echo "[$(date)] Starting F-exp05: config=${CONFIG}, resume=foreact-pretrained, 5 epochs"
