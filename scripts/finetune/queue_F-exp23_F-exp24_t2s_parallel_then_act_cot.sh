...
start_hold() {
  local log_file="logs/aloha_fexp/act_cot_hold_after_t2s_${RUN_TS}.log"

  echo "[$(date '+%Y-%m-%d %H:%M:%S')] both t2s experiments finished; starting act_cot hold on all visible GPUs; log=${log_file}"
...
echo "[$(date '+%Y-%m-%d %H:%M:%S')] F-exp23 t2s GPUs: ${gpus_exp23}"
echo "[$(date '+%Y-%m-%d %H:%M:%S')] F-exp24 t2s GPUs: ${gpus_exp24}"

run_experiment \
  "F-exp23-eggplant-potato-gripper-binary_motion_delta005_s5_t2s.yaml" \
  "26025" \
  "F-exp23-eggplant-potato-gripper-binary_motion_delta005_s5_t2s" \
  "$gpus_exp23" &
pid_exp23=$!

run_experiment \
  "F-exp24-aloha-banana-fix-cube-eggplant-potato_motion_delta005_s5_t2s.yaml" \
  "26026" \
  "F-exp24-aloha-banana-fix-cube-eggplant-potato_motion_delta005_s5_t2s" \
  "$gpus_exp24" &
pid_exp24=$!
...
if (( status_exp23 != 0 || status_exp24 != 0 )); then
  echo "[$(date '+%Y-%m-%d %H:%M:%S')] t2s training failed: F-exp23=${status_exp23}, F-exp24=${status_exp24}; act_cot hold will not start" >&2
  exit 1
fi
