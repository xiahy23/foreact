...
def get_output_video_path(base_dir, episode_id, suffix="foreact"):
    chunk_idx = episode_id // 1000
    return os.path.join(
        './output_videos',
        f"chunk-{chunk_idx:03d}",
        f"observation.images.image_0_{suffix}",
        f"episode_{episode_id:06d}.mp4",
    )
...
def main():
    ...
    parser.add_argument("--checkpoint_path", type=str,
                        default="./checkpoints/automl/trial_403/trial_403_lr1.4e-05_bs8_const_ws100/checkpoint-7510")
    ...
    # all_episode_ids = all_episode_ids[22000:]
    ...
    for ep_i, episode_id in enumerate(my_episode_ids):
        output_path_6 = get_output_video_path(args.video_dir, episode_id, "foreact6")
        output_path_7 = get_output_video_path(args.video_dir, episode_id, "foreact7")

        if os.path.exists(output_path_6) and os.path.exists(output_path_7):
            episodes_done += 1
            continue
        ...
        for frame_idx in range(num_frames):
            ...
            with torch.no_grad():
                _, intermediates = pipeline.sample_images(
                    caption=prompt,
            ...
            # Step 6
            img_6 = intermediates[6][0]
            if img_6.size != target_size:
                img_6 = img_6.resize(target_size, resample=resample)
            pred_frames_6.append(np.array(img_6, dtype=np.uint8))

            # Step 7
            img_7 = intermediates[7][0]
            if img_7.size != target_size:
                img_7 = img_7.resize(target_size, resample=resample)
            pred_frames_7.append(np.array(img_7, dtype=np.uint8))

            total_frames_processed += 1

        for suffix, frames_list in [("foreact6", pred_frames_6), ("foreact7", pred_frames_7)]:
            out_path = get_output_video_path(args.video_dir, episode_id, suffix)
            os.makedirs(os.path.dirname(out_path), exist_ok=True)
            pred_tensor = torch.from_numpy(np.stack(frames_list, axis=0))
